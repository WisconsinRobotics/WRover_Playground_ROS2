...
class StatusLight:
    """Class for drawing the status light on the canvas."""
    ...
    def __init__(self, canvas: tk.Canvas, reached_target: bool = False):
        ...
        self.frame = ttk.Frame(
            self.canvas, width=FRAME_DIM[0], height=FRAME_DIM[1], style='StatusFrame.TFrame')
        self.frame['padding'] = 5

        # Create status heading
        s.configure('StatusLabel.TLabel', background='white', anchor=tk.NW)
        self.status_heading = ttk.Label(
            self.frame, text='Robot Status', font=('TkHeadingFont', 16),
            style='StatusLabel.TLabel')

        # Create status light
        self.light_canvas = tk.Canvas(
            self.frame, width=LIGHT_DIMEN[0] + 5, height=LIGHT_DIMEN[1] + 5, bg='white')
        self.light_canvas.configure(highlightthickness=0, borderwidth=0)
        self.status_light = self.light_canvas.create_oval(
            0, 0, *LIGHT_DIMEN, fill='red', outline='black')

        # Create status message
        self.status_msg = ttk.Label(
            self.frame, text='Navigating to target', style='StatusLabel.TLabel')
        ...
        self.canvas.create_window(
            *FRAME_TOP_LEFT, anchor=tk.NW, window=self.frame)

    def get_reached_target(self) -> bool:
        """Return true if the status light is set to done, false otherwise."""
        return self.__reached_target

    def set_reached_target(self):
        """Set the status light to done."""
    ...
    def set_navigating_to_target(self):
        """Set the status light to not done."""
